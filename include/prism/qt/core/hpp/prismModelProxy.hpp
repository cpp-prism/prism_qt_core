...
template<class T>
void getTransValue(const char* fname,T& model,QVariant& qv)
{
...
public:
    ...
    explicit prismModelProxyBase(QObject* parent =nullptr):QObject(parent) {
...
public slots:
    ...
    Q_INVOKABLE virtual   QVariant get(const QByteArray& fname) { Q_UNUSED(fname) return QVariant(); }
    Q_INVOKABLE virtual  void set(const QByteArray& fname,QVariant value){ Q_UNUSED(fname) Q_UNUSED(value) }
...
template<class TV> constexpr inline bool set_field_do(QList<const char*>& fns,TV& p_v, QVariant& value)
{
    ...
    if(fns.length()==0)
    {
        ...
        {
            qDebug()<< "设置值失败,类型错误,不能把" << value << QString("设置给[%1]类型的变量")
                       .arg(QString::fromStdString(std::string(utilities::typeName<decltype(p_v)>::name())));
            return false;
        }
