...
class PRISMQT_CORE_EXPORT prismModelListProxyBase:public QAbstractListModel
{
   Q_OBJECT
public:
    prismModelListProxyBase(QObject* parent =nullptr){ Q_UNUSED(parent) }
    virtual ~prismModelListProxyBase(){}
    Q_INVOKABLE virtual  QVariant getRowData(int index) { Q_UNUSED(index) return QVariant(); }
    Q_INVOKABLE virtual  QVariant getRowData(const QModelIndex& index) { Q_UNUSED(index) return QVariant(); }
    Q_INVOKABLE virtual int length() { return 0; }
    Q_INVOKABLE virtual int indexOfValue(QString fname ,QVariant value){ Q_UNUSED(fname)  Q_UNUSED(value) return 0;}
...
public:
    using value_type = T;
    explicit prismModelListProxy(QObject* parent = nullptr) :prismModelListProxyBase(parent),  m_list(new QVector<prismModelProxy<T>*>())
    {
        Q_UNUSED(parent)
    ...
    QVector<prismModelProxy<T>*>* list() const { return m_list; }
